...
saveto = '../../Figures/LatB_Contour_Hist';
...
% meandian  - mean diameter from all measurments with given method
...
binS_Dia = [0.5; 0.5; 0.4];
binS_Def = [5.6; 0.0075; 0.18];
binS_Def_Sel = [3; 0.014; 0.35];

% define plotting limits
size_min_sets = [10; 10; 10];
size_max_sets = [18; 18; 18];

D_min_sets = [0; 0.02; 1.5];
D_max_sets = [90; 0.16; 4.5];
...
% prepare contour
for ii = 1:length(LatB)
    X = data(data(:, 1) == LatB(ii), 4);
    Y = data(data(:, 1) == LatB(ii), 6);
    if ~isempty(X)
...
values1 = data(:, diam_ind(m));
for bb = 1:length(LatB)
    values = values1(find(data(:, 1) == LatB(bb)));
    for ii = 1:barN
...
values1 = data(:, def_ind(m));
values1 = values1(selind);
LatB1 = data(:, 1);
LatB1 = LatB1(selind);
for bb = 1:length(LatB)
    values = values1(LatB1 == LatB(bb));
...
legend(cellstr(num2str(LatB*1000)), 'FontSize', 16)
text(0.92*size_max, 0.55*D_max, 'ng ml^{-1}', 'FontSize', 16)
legend('boxoff')
...
print(fullfile(saveto, ['LatB_SMR_Contour_Hist_Set', num2str(aa)]), '-dtiff', '-r300')

%% SMR - plot with demnostrating bin selection, Supplementary Figure 6a
...
print(fullfile(saveto, ['LatB_SMR_Contour_Hist_Set', num2str(aa), '_BinSel']), '-dtiff', '-r300')
...
%prepare contours
for ii = 1:length(LatB)
    X = data(data(:, 1) == LatB(ii), indS);
    Y = data(data(:, 1) == LatB(ii), indD);
    [plcor50{ii}, ~] = contour50(X, Y, 'bins', [100, 100]);
    close
    clear X Y
end
...
%change '-dtiff' for '-depsc' for vector graphic version
print(fullfile(saveto, ['LatB_RTDC_Contour_Hist_Set', num2str(aa)]), '-dtiff', '-r300')

%% RTDC - plot with demnostrating bin selection, Supplementary Figure 6b
...
%change '-dtiff' for '-depsc' for vector graphic version
print(fullfile(saveto, ['LatB_RTDC_Contour_Hist_Set', num2str(aa), '_BinSel']), '-dtiff', '-r300')
...
m = 3; % method's index
aa = 1; % experimental series selected for plotting
data = sets{m}{aa};
...
ylabel('deformability', 'FontSize', 18);
legend(cellstr(num2str(LatB*1000)), 'FontSize', 16)
text(0.92*size_max, 0.7*D_max, 'ng ml^{-1}', 'FontSize', 16)
legend('boxoff')
xlim([size_min, size_max])
ylim([D_min, D_max])
box on
...
%change '-dtiff' for '-depsc' for vector graphic version
print(fullfile(saveto, ['LatB_DC_Contour_Hist_Set', num2str(aa)]), '-dtiff', '-r300')

%% DC - plot with demnostrating bin selection, Supplementary Figure 6c
...
%change '-dtiff' for '-depsc' for vector graphic version
print(fullfile(saveto, ['LatB_DC_Contour_Hist_Set', num2str(aa), '_BinSel']), '-dtiff', '-r300')
