...
methods = {'SMR', 'RTDC', 'DC'};
set_sel = [2; 4; 1]; % index of selected set for each method
...
for m = 1:length(methods)
    ...
    f = figure
    violin = violinplot(data(:, diam_ind(m)), data(:, 1).*1000, 'ShowData', false, ...
    ...
    xlabel(['LatB concentration (ng ml^{-1})'], 'Interpreter', 'tex')
    ylim([5, 25])
    ...
    print(strcat(saveto, 'LatB_Violin_', methods{m}, '_Size_Set', num2str(set_sel(m)), '.tiff'), '-dtiff', '-r300')
...
for m = 1:length(methods)
    ...
    selindcnt = intersect(selind, find(data(:, 1) == 0));
    defmedcnt(m, 1) = median(data(selindcnt, def_ind(m)));
    
    f = figure
    violin = violinplot(data(selind, def_ind(m))./defmedcnt(m, 1), data(selind, 1).*1000, 'ShowData', true, ...
    ...
    set(gcf, 'PaperUnits', 'centimeters');
    set(gcf, 'PaperPosition', [0, 0, 16, 15]);
    print(strcat(saveto, 'LatB_Violin_', methods{m}, '_RD_Set', num2str(set_sel(m)), '.tiff'), '-dtiff', '-r300')
