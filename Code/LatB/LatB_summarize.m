% this function prepares data summary of the LatB experiments

function [summary, factor_all, summary_headers, table] = LatB_summarize(sets, diam_ind, def_ind, mean_diam, LatBL, LatBLStr)
% Loop through SMR, RTDC, DC data and generate summaries
% each row contains median values for individual measurment

summary_headers = {'LatB', 'Deformability', 'Relative_Deformability', 'Exp', 'Deformability_All', 'Relative_Deformability_All', 'Size', 'n_all', 'n_sel'};
...
for i = 1:3
    ...
    for ii = 1:length(set0)
        set = set0{ii};
        indsel1 = find(set(:, diam) >= mean_diam(i)-0.5 & set(:, diam) <= mean_diam(i)+0.5);
        setsel1 = set(indsel1, :);
        indcnt = find(set(:, 1) == 0);
        ...
        for cc = 1:length(check)
            ...
            factor0{cc, 1} = LatBLStr(class(cc, 1));
...
for aa = 1:length(summary)
    ...
    table{aa} = [LatBL, meanall, SDall, mean1, SD1, meanS, SDS, n];
end
